		var _wm_settings = {
		    popunder: {
		        url: 'http:\/\/prpops.com\/p\/cif7\/direct',
		        times: 1,
		        hours: 6,
		        cookie: 'prPop583747',
		        fastbind: true
					...
					if(widgets[i].getAttribute('data-res')=='true'){
						...
						if(containerHeight <= 1) {
							...
							if (typeof(h) != 'undefined' && h > 0) {
								widgets[i].setAttribute('style', 'height:'+h+'px;');
								containerHeight = h;
							} else {
								containerHeight = 1;
							}
						...
						if(containerWidth <= 1) {
							...
							if (typeof(w) != 'undefined' && w > 0) {
								widgets[i].setAttribute('style', 'width:'+w+'px;');
								containerWidth = w;
							} else {
								containerWidth = 1;
							}
						}
						el.setAttribute('src','//prwidgets.com/adultubeus.info/'+id+'/'+containerWidth+'/'+containerHeight);
					}else{
						el.setAttribute('height',parseInt(widgets[i].style.height));
						el.setAttribute('width',parseInt(widgets[i].style.width));
						el.setAttribute('src','//prwidgets.com/adultubeus.info/'+id);
					}
					widgets[i].setAttribute('added','true');
					setTimeout(function(elements,element){
		...
		if(typeof(sliders)!='undefined'&&sliders.length>0){
			for(var i=0;i < sliders.length;i++){
				...
				if(typeof(id)!='undefined'&&sliders[i].getAttribute('added')!=='true'){
					...
					el.setAttribute('src','//prsliders.com/adultubeus.info/'+id);
